...
import pandas as pd
#For unpickle error run the below: 
...
# Add "age" as numeric encoding in encoding_mappings
encoding_mappings["age"] = {"Valid Age Range": 1.0}  # Placeholder value
...
def map_numeric_response(question, response):
    column = question_to_column.get(question)
    response_lower = response.lower().strip()

    if column in encoding_mappings:
        for key, value in encoding_mappings[column].items():
            if response_lower in key.lower():
                return value
    ...
    age_mappings = {
        "age": [(8, 21, "Valid Age Range")],
        "age of first whole cigarette": [
            (0, 8, "8 years old or younger"),
            (9, 10, "9 or 10 years old"),
            (11, 12, "11 or 12 years old"),
            (13, 14, "13 or 14 years old"),
            (15, 16, "15 or 16 years old"),
            (17, float('inf'), "17 years old")
        ],
        "age of first drink": [
            (0, 8, "8 years old or younger"),
            (9, 10, "9 or 10 years old"),
    ...
    if column in age_mappings:
        for lower, upper, category in age_mappings[column]:
            if lower <= num <= upper:
                return encoding_mappings[column][category]

    frequency_mappings = {
        "drink and drive in last 30 days": [
            (0, 0, "0 times"), (1, 1, "1 time"),
            (2, 3, "2 or 3 times"), (4, 5, "4 or 5 times"),
            (6, float('inf'), "6 or more times")
        ],
        "days smoked cigarettes": [
            (0, 0, "0 days"), (1, 2, "1 or 2 days"),
            (3, 5, "3 to 5 days"), (6, 9, "6 to 9 days"),
            (10, 19, "10 to 19 days"), (20, 29, "20 to 29 days"),
            (30, float('inf'), "All 30 days")
        ],
        "days used chewing tobacco": [
        (0, 0, "0 days"), (1, 2, "1 or 2 days"),
        (3, 5, "3 to 5 days"), (6, 9, "6 to 9 days"),
        (10, 19, "10 to 19 days"), (20, 29, "20 to 29 days"),
        (30, float('inf'), "All 30 days")
    ...
    "how many days did you have 5+ drinks": [
        (0, 0, "0 days"), (1, "1 day"),
        (2, 2, "2 days"), (3, 5, "3 to 5 days"),
        (6, 9, "6 to 9 days"), (10, 19, "10 to 19 days"),
        (20, float('inf'), "20 or more days")
    ],
    "lifetime weed use": [
        (0, 0, "0 times"), (1, 2, "1 or 2 times"),
        (3, 9, "3 to 9 times"), (10, 19, "10 to 19 times"),
        (20, 39, "20 to 39 times"), (40, 99, "40 to 99 times"),
        (100, float('inf'), "100 or more times")
    ],
    "needle usage": [
        (0,  0, "0 times"), (1, "1 time"),
        (2, float('inf'), "2 or more times")
    ],
    "cocaine usage": [
        (0, 0, "0 times"), (1, 2, "1 or 2 times"),
        (3, 9, "3 to 9 times"), (10, 19, "10 to 19 times"),
        (20, 39, "20 to 39 times"), (40, float('inf'), "40 or more times")
    ],
    "heroin usage": [
        (0, 0, "0 times"), (1, 2, "1 or 2 times"),
        (3, 9, "3 to 9 times"), (10, 19, "10 to 19 times"),
    ...
    "lifetime cigarette count": [
        (0, 0, "0 cigarettes"), (1, "1 cigarette"),
        (2, 5, "2 to 5 cigarettes"), (6, 15, "6 to 15 cigarettes"),
        (16, 25, "16 to 25 cigarettes"), (26, 99, "26 to 99 cigarettes"),
        (100, float('inf'), "100 or more cigarettes")
    ],
     "times used weed": [
        (0, 0, "0 times"), (1, 2, "1 or 2 times"),
        (3, 9, "3 to 9 times"), (10, 19, "10 to 19 times"),
    ...
    if column in frequency_mappings:
        for lower, upper, category in frequency_mappings[column]:
...
def preprocess_for_model(response_history: List[str]) -> np.array:
    ...
    for question, user_response in zip(questions, response_history):
        encoded_value = map_numeric_response(question, user_response)
        if encoded_value is None:
            invalid_inputs.append(f"Q: {question}, A: {user_response}")
            encoded_responses.append(-1)
        else:
            encoded_responses.append(encoded_value)
    
    if invalid_inputs:
        raise ValueError(f"Invalid inputs detected: {invalid_inputs}")
    ...
    if question_index == 0:
        ...
        if question_index <= len(questions):
            ...
            try:
                ...
                model_input_df = pd.DataFrame(model_input, columns=rf_model.feature_names_in_)
                predicted_risk = rf_model.predict(model_input_df)[0]
                ...
                await cl.Message(content=f"Error: {str(e)}. Please restart the process.").send()

            cl.user_session.set("question_index", 0)
            cl.user_session.set("user_responses", [])

#chainlit run new.py -w
